...
export default function BentoMenu() {
  ...
  useEffect(() => {
    fetch(`${apiendpoint}api/devices/dashboard/totalDevices`, {
      credentials: "include",
    })
      .then((res) => {
        if (!res.ok) {
    ...
    fetch(`${apiendpoint}api/devices/dashboard/totalUnderRepair`, {
      ...
      .then((res) => {
        if (!res.ok) {
          throw new Error(
            "Errore nel recupero del totale dei dispositivi sotto riparazione"
          );
        }
        return res.json();
      })
        ...
        <div className="bento-box col-span-2 row-span-1 rounded-xl bg-gradient-to-b from-yellow-300 via-white-200 to-orange-500 p-2 sm:p-4 flex flex-col justify-center">
          <div className="flex justify-between">
            <div
              ...
              onClick={redirectToDeviceTypes}
            >
              <p className="bento-title font-bold">Totale tipi di dispositivi</p>
            ...
            >
              <p className="bento-title font-bold">
                Dispositivi con garanzia valida non assegnati
              </p>
